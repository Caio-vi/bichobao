...
const indexfotos = [
 'imagens/Avelar Fotografia-1622.jpg ', 'imagens/Avelar Fotografia-1795.jpg ', 'imagens/Avelar Fotografia-1817.jpg'
];
...
function proximaimagem(){
    ...
    // ✅ CORREÇÃO: usa sobrefotos.length (não indexfotos.length)
...
// ✅ APENAS UM intervalo (remove o duplicado)
setInterval(proximaimagem, 3000);

window.addEventListener('load', function() {
    const slides = document.getElementsByClassName("slide");
    for (let i = 0; i < slides.length; i++) {
        // ✅ CORREÇÃO: usa sobrefotos[0] (não indexfotos[0])
...
function agendarWhatsApp(servico) {
  ...
  const url = `https://wa.me/${"5535998181384"}?text=${encodeURIComponent(
    mensagem
  )}`;
